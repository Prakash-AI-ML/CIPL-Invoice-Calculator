...
async function saveStatement(data, filename) {
    modal_id_for_print=data.extract_id;
    ...
    const $iframe_download = $('#' + modal_id_for_print + 'Body');
        // const $iframe_download = $('#' + modal_id_for_print ).find('iframe');
          if(data.template_id>0 && pay_term_status && category_status && modal_id_for_print!=""){
...
data.savedInvoice="true";
     $('.'+modal_id_for_print+'DownloadOPtion').removeClass('hide');
        $('.'+modal_id_for_print+'PreviewOPtion').removeClass('hide');
        $('.'+modal_id_for_print+'saveOption').addClass('hide');
       var iframe_download = $iframe_download.find('iframe').get(0);

if (!iframe_download) {
  console.error('Dynamic iframe not found yet');
  return;
}
...
refreshIframeFromCurrentContent($iframe[0],data);
// refreshIframeFromCurrentContent(iframe_download,data);
// $('.'+modal_id_for_print+'DownloadOPtion').data('onclick',`generatePDFFromIframe(${iframe_download}, ${filename})`);
// $('.' + modal_id_for_print + 'DownloadOPtion')
//   .off('click')   // prevent duplicate bindings
//   .on('click', function () {
//     generatePDFFromIframe(iframe_download, filename);
//   });
iframe_download = await refreshIframeFromCurrentContent(iframe_download, data);
...
async function openExtractModal(data, prefill = {}) {
    document.getElementById("modal_payment_term").value = "";
document.getElementById("modal_merchant_category").value = "";
document.getElementById("modal_client_address").value = "";
document.getElementById("modal_client_id").value = "";
document.getElementById("modal_client_name").value = "";
document.getElementById("modal_client_phone").value = "";
document.getElementById("modal_client_bankName").value = "";
document.getElementById("modal_client_bankAcc").value = "";
document.getElementById("modal_client_bankOrigin").value = "";
document.getElementById("modal_client_shiftCode").value = "";
document.getElementById("modal_client_ibanNo").value = "";
    ...
    return new Promise(async (resolve) => {
        ...
        const modalEl = document.getElementById("extractModal");
           const templatePreview = document.getElementById("templatePreviewDiv");
           
           let htmlText = await fetch(`/static/app/7templates.html`).then(r => r.text());
           htmlText = await injectSampleData(htmlText, data);
      templatePreview.srcdoc = htmlText;
        ...
        document.querySelectorAll(".modal_cancel_btn").forEach(btn => {
  btn.onclick = () => {
    resolve({ action: "cancel" });
  };
});
        ...
        document.getElementById("modal_save_btn").onclick = async () => {
            ...
            const bank_name = document.getElementById("modal_client_bankName").value.trim();
            const bank_acc = document.getElementById("modal_client_bankAcc").value.trim();
            const currency = document.getElementById("modal_client_bankOrigin").value.trim();
            const shift_code = document.getElementById("modal_client_shiftCode").value.trim();
            const iban_no = document.getElementById("modal_client_ibanNo").value.trim();

            const template_id = sessionStorage.getItem('selected_template') || 1;
            
            if (!term || !cat || !address_ || !customer_id || !client_name || !template_id) {
            ...
            const payload = {
                ...
                bank_acc_number: bank_acc || null,
                bank_origin: currency || null,
                iban_no: iban_no || null,
                swift_code: shift_code || null,
            };
...
function parseDate(dateStr) {
 ...
 function saveClientDetails(save_id,modalId,ele,data) { 
    ...
    data.term = payload.pay_term;
    data.merchant_category = payload.merchant_category;
    data.template_id = payload.template;
    data.pay_term_status = true;
...
buttonTabs.forEach(el => {
  el.classList.add('hide');
    ...
    onclick="if(saveClientDetails('${save_id}', 'pdfPreviewModal${modalId}',this,${JSON.stringify(data)})!=false) { $('.${save_id}').trigger('click');  }" 
...
function createInvoiceCard(data, filename) {
    ...
    data.extract_id=modal_id_for_print; // For bulk save
...
createpreviewCard(data, filename,card,$id,modal_id_for_print);
  ...
  ${canShowAction('save') ? `
    ...
    <button 
  class="btn btn-light btn-sm ${modal_id_for_print}_save_all hide "  
  onclick='save_the_details(${JSON.stringify(data)}, "${modal_id_for_print}","${$id}",this)'>

      <i class="bi bi-eye"></i> Save
    </button>
    <button class="btn btn-light btn-sm me-2 save-single  ${modal_id_for_print}saveOption   ${$id}" title="Save this statement">
      <i class="bi bi-cloud-check"></i> Save
    </button>
...
btn.remove(); // removes the element from the DOM
...
function addBulkSaveButton1() {
  ...
  `;
    ...
    for (const { button, handler } of actions) {
        try {
            const result = handler.call(button);

            const resolved = result instanceof Promise
                ? await result
                : result;
    ...
    `;
        ...
        document.getElementById('bulkSaveBtn').addEventListener('click', async () => {
            ...
            var result;
            if(!data.savedInvoice){
                result = await saveStatement(data, filename);
        ...
        `;
            ...
            showToast(
             ...
             showToast(
                `ALL Statements are already saved`,
